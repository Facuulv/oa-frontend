...
const PRODUCTOS_ENDPOINT = `${API_BASE}/public/productos`;

const ICE_BAG_PRICE = 2500;
...
const isPromocionesCategory = (raw) => {
  ...
  const slug = normalizeTextKey(
    raw.categoria_slug ?? raw.category_slug ?? raw.slug_categoria
  );
  return (
    nombre === "promociones" ||
    nombre === "promocion" ||
...
const hasPrebuiltComponents = (raw) => {
  const list =
    raw.productos_componentes ?? raw.componentes ?? raw.items_componentes;
 ...
 */
const isPrebuiltPromoOrCombo = (raw) => {
  ...
  if (
    isTruthyPromoFlag(
      raw.is_promo ?? raw.isPromo ?? raw.es_promo ?? raw.esPromo
    )
  ) {
    return true;
  }
...
const mapSelectableProduct = (raw) => {
